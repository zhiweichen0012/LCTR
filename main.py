...
def get_args_parser():
    ...
    parser.add_argument('--vis', default=False, help='vis')
    ...
    parser.add_argument(
        '--finetune',
        default='weight/deit_small_patch16_224.pth',
    ...
    parser.add_argument(
        '--output_dir', default='./ckpt', help='path where to save, empty for no saving'
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.eval:
        ...
        if args.data_set == 'IMNET':
            threshold_loc_list = [0.18]
        else:
            threshold_loc_list = [0.09]

        for _th in threshold_loc_list:
            test_stats = evaluate(
                data_loader_val, model, device, args, threshold_loc=_th
            )
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) == args.epochs:
            # if ((epoch + 1) % save_epoch == 0) and ((epoch + 1) > args.epochs * 0.6):
            checkpoint_path = os.path.join(
                args.output_dir, 'checkpointAt' + str(epoch + 1) + '.pth'
            )
